...
export default function ProfileHeader(viewedUser, className, postsCount) {
    ...
    const profileImageContent = (viewedUser && viewedUser.profile_picture_url) 
        ? `<img src="${viewedUser.profile_picture_url}" alt="${viewedUser.username}" class="w-full h-full object-cover">`
        : `<div class="w-full h-full flex items-center justify-center text-gray-500 text-4xl font-bold">${viewedUser ? viewedUser.username.slice(0, 2).toUpperCase() : '?'}</div>`;
    ...
    <div class="flex-shrink-0">
        ...
        <div class="flex-1 flex flex-col items-center md:items-start gap-4 w-full">
            <div class="flex flex-col md:flex-row items-center gap-4">
                <h1 class="text-xl md:text-2xl font-normal">${viewedUser ? viewedUser.username : 'username'}</h1>
            ...
            <div class="text-sm md:text-base text-center md:text-left">
                <div class="font-bold">${viewedUser ? viewedUser.fullname : 'Full Name'}</div>
                <div class="text-gray-300">${viewedUser && viewedUser.bio ? viewedUser.bio : ""}</div>
            </div>
        </div>
    `;
